...
def take_snapshot(name):
    ...
    from utils.package.repoutils import getRevision
    ...
    for package in packages:
        ...
        stream = open(path, 'a')
        print >> stream, 'revision=%s' % rev
        print >> stream, 'repo = repoutils.svn.getPackageRepository(reponame, branch, revision=revision, name=name)'
        continue
    return
